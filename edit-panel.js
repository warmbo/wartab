...
function openCardEditPanel(cardId) {
  ...
  document.body.classList.toggle('panel-left', panel.classList.contains('slide-left'));
...
function updateBlurState() {
  ...
  document.body.classList.toggle('panel-left', _editPanelOpen && !configPanelOpen && $('#edit-panel').classList.contains('slide-left'));
}
...
function saveAndRefresh(structural) {
  ...
  if (structural && _editingCardId) {
    ...
    const card = config.cards.find(c => c.id === _editingCardId);
    if (card) {
      const newEl = rerenderCard(card);
      if (newEl) {
        // Re-highlight the new card element (lost on replaceWith)
        newEl.classList.add('card-highlight');
      }
      const title = $('#edit-panel-title');
      if (title) title.textContent = '✎ ' + (card._isGap ? 'Edit Gap' : escHtml(card.title || 'Untitled'));
    }
...
function buildCardEditPanel(card) {
  ...
  if (card._isGap) {
    ...
    body.appendChild(cpCheck('Empty gap (no content)', true, function(v) {
      if (!v) { card.sections = card._savedSections || [WarTabCardModel.createSection('links')]; }
      card._isGap = v;
  ...
  if (card.icon) ip.appendChild(ds.icon(card.icon, 'config-icon-glyph'));
  ...
  addSecBtn.addEventListener('click', function() {
    ...
    card.sections.push(WarTabCardModel.createSection('links'));
    saveAndRefreshStructural();
    toast('Section added');
  });
