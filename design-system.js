...
/** Render a configured Lucide name, image URL/path, or emoji as DOM. */
ds.icon = function(value, cls, fallback) {
  var icon = value || fallback;
  if (!icon) return null;
  ...
  glyph.className = ((cls || '') + ' emoji-icon').trim();
...
ds.empty = function(icon, message, hint, action) {
  ...
  // Icon
  var iconEl = _el('div', 'ds-empty-icon');
  iconEl.appendChild(ds.icon(icon, 'ds-empty-icon-glyph', 'package'));
  ...
  if (opts.icon || opts.title || opts.status || opts.actions) {
    ...
    if (opts.icon) {
      if (typeof opts.icon === 'string') {
        hdr.appendChild(ds.icon(opts.icon, 'ds-module-icon'));
