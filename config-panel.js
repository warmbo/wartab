...
async function restoreConfigSnapshot(name) {
  // Saving the current snapshot acts as a queue barrier: restore cannot race an older write.
  await saveConfig();
  await storage.snapshots.restore(name);
  await loadConfig();
  pageInit();
  return config;
}
...
function buildDashboardPanel(body){
  ...
  ip.appendChild(ds.icon(bi,'config-icon-glyph','sword'));
  ir2.appendChild(ip);
  ...
  if (_cp) {
    ...
    piPrev.appendChild(ds.icon(_cp.icon,'config-icon-glyph','layout'));
...
if(v==='gradient'){if(!config.theme.bgValue.includes(','))config.theme.bgValue='#0a0a0a, #1a1a1a';}else if(v==='solid'){var parts=config.theme.bgValue.split(',');config.theme.bgValue=(parts[0]||'#0a0a0a').trim();}else if(v==='image'&&config.theme.bgValue.includes(',')){config.theme.bgValue='#0a0a0a';}applyChanges();renderAll();buildConfigPanel();}));
  ...
  /* Top Bar */
  body.appendChild(ps('Top Bar'));
  body.appendChild(pf('range','','Icon & Text Scale',null,Math.round((parseFloat(config.theme.topBarScale)||1)*10),v=>{config.theme.topBarScale=parseInt(v)/10;applyChanges();},{min:5,max:20}));
...
function buildSystemPanel(body){
  ...
  ['Export','Import','Reset'].forEach(label=>{
    ...
    b.addEventListener('click',()=>{
      ...
      else if(label==='Reset'){showConfirmModal('Reset all settings to defaults? This cannot be undone.',()=>{const snap=cloneObj(config);config=cloneObj(DEFAULT_CONFIG);pageInit();saveConfig();applyTheme();renderAll();_configTab='system';buildConfigPanel();initStatusBar();toastWithUndo('Reset',()=>{config=snap;pageInit();saveConfig();applyTheme();renderAll();_configTab='system';buildConfigPanel();initStatusBar();});},'Reset');}
    });
    acts.appendChild(b);
  });
  ...
  fi2.addEventListener('change',e=>{if(e.target.files[0]){const r=new FileReader();r.onload=async ev=>{try{const d=JSON.parse(ev.target.result);var result=sanitizeImportConfig(d);var warnings=result.warnings;var cleaned=result.data;showConfirmModal('Import config from '+e.target.files[0].name+'? This will replace your current configuration.',async()=>{try{await persistImportedConfig(cleaned);}catch(e){toast('Import save failed: '+e.message,'error');return;}applyTheme();renderAll();_configTab='system';buildConfigPanel();initStatusBar();if(warnings.length){showModal('Import completed with ' + warnings.length + ' warnings', warnings);}else{toast('Imported');}},'Import');}catch(e){toast('Failed: '+e.message,'error');}};r.readAsText(e.target.files[0]);}});
  ...
  function renderSnapshots(){
    ...
    storage.snapshots.list().then(snaps=>{
      ...
      snaps.forEach(s=>{
        ...
        rst.addEventListener('click',()=>{showConfirmModal('Restore snapshot from '+ts+'? Current config will be replaced.',async()=>{try{await restoreConfigSnapshot(s.name);applyTheme();renderAll();_configTab='system';buildConfigPanel();initStatusBar();toast('Restored: '+ts);}catch(e){toast('Restore failed: '+e.message,'error');}}, 'Restore')});
