...
(function () {
  ...
  function initShell(){
    ...
    var more=document.getElementById('btn-more'),menu=document.getElementById('command-deck-menu');if(more&&menu){var hasPopover=typeof menu.showPopover==='function';if(!hasPopover)menu.hidden=true;var toggleMore=function(){var open=hasPopover?menu.matches(':popover-open'):!menu.hidden;if(open){if(hasPopover)menu.hidePopover();else menu.hidden=true;}else{if(hasPopover){var rect=more.getBoundingClientRect();menu.style.top=Math.min(window.innerHeight-360,rect.bottom+8)+'px';menu.style.left=Math.max(8,rect.right-220)+'px';menu.showPopover();}else menu.hidden=false;}more.setAttribute('aria-expanded',String(!open));};more.onclick=function(e){e.stopPropagation();toggleMore();};if(hasPopover)menu.addEventListener('toggle',function(e){more.setAttribute('aria-expanded',String(e.newState==='open'));});menu.onclick=function(e){var a=e.target.closest('[data-command]');if(!a)return;if(hasPopover)menu.hidePopover();else menu.hidden=true;var action=a.dataset.command;if(action==='pages')openPageManagementPanel();else if(action==='arrange')toggleArrangeMode();else if(action==='layout')openLayoutStudio();else if(action==='edit')enterContextualEditMode();else if(action==='presets')openExperiencePresets();else if(action==='settings')toggleConfigPanel();else if(action==='help')showShortcutsOverlay();};}
