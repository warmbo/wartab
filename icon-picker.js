...
let _iconPickerReturnFocus=null,_iconPickerObserver=null;
function makeIconChoice(el,label){el.setAttribute('role','button');el.setAttribute('tabindex','0');if(label)el.setAttribute('aria-label',label);el.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();el.click();}});return el;}
function openIconPicker(cb){_iconPickerReturnFocus=document.activeElement;iconPickerCallback=cb;iconPickerOpen=true;$('#icon-picker-overlay').classList.add('open');$('#icon-picker').classList.add('open');buildIconPicker('icons');setTimeout(function(){$('#icon-picker').querySelector('input,.ip-tab,button')?.focus();},0);}
function closeIconPicker(){iconPickerOpen=false;iconPickerCallback=null;if(_iconPickerObserver){_iconPickerObserver.disconnect();_iconPickerObserver=null;}$('#icon-picker-overlay').classList.remove('open');$('#icon-picker').classList.remove('open');if(_iconPickerReturnFocus&&document.contains(_iconPickerReturnFocus))_iconPickerReturnFocus.focus();_iconPickerReturnFocus=null;}
function buildIconPicker(t){if(_iconPickerObserver){_iconPickerObserver.disconnect();_iconPickerObserver=null;}const c=$('#icon-picker-content');c.innerHTML='';$$('.ip-tab').forEach(x=>{const active=x.dataset.tab===t;x.classList.toggle('active',active);x.setAttribute('aria-selected',String(active));});if(t==='library')buildLibraryTab(c);else if(t==='upload')buildUploadTab(c);else if(t==='icons')buildIconsTab(c);else if(t==='url')buildUrlTab(c);}
function buildLibraryTab(c){const s=document.createElement('input');s.className='icon-search-bar';s.placeholder='Search services...';c.appendChild(s);const g=document.createElement('div');g.className='icon-grid';c.appendChild(g);_iconPickerObserver=new MutationObserver(function(){g.querySelectorAll('.icon-grid-item:not([role])').forEach(function(d){makeIconChoice(d,d.textContent);});});_iconPickerObserver.observe(g,{childList:true});let _riTimer=null;function ri(f){g.innerHTML='';const fl=(f||'').toLowerCase();if(!ICON_REPO.length){g.innerHTML='<div style="grid-column:1/-1;padding:20px;text-align:center;color:var(--text-tertiary);font-size:var(--text-base);">Loading service icons...</div>';loadIconRepo();if(!_riTimer)_riTimer=setInterval(function(){if(ICON_REPO.length){clearInterval(_riTimer);_riTimer=null;ri(s.value);}else{loadIconRepo();}},300);return;}if(_riTimer){clearInterval(_riTimer);_riTimer=null;}const items=fl?ICON_REPO.filter(i=>i.name.toLowerCase().includes(fl)||i.file.toLowerCase().includes(fl)||i.tags.some(t=>t.includes(fl))):ICON_REPO;items.slice(0,120).forEach(item=>{const d=document.createElement('div');d.className='icon-grid-item';const img=document.createElement('img');img.src=`${ICON_CDN}/${item.file}.svg`;img.alt=item.name;img.onerror=function(){this.parentElement.style.display='none';};const l=document.createElement('span');l.textContent=item.name;d.appendChild(img);d.appendChild(l);d.addEventListener('click',()=>selectIcon(`${ICON_CDN}/${item.file}.svg`));g.appendChild(d);});if(!items.length)g.innerHTML='<div style="grid-column:1/-1;padding:20px;text-align:center;color:var(--text-tertiary);font-size:var(--text-base);">No icons found</div>';}s.addEventListener('input',()=>ri(s.value));ri('');}
function buildUploadTab(c){
  c.innerHTML='';
  const z=makeIconChoice(document.createElement('div'),'Upload an icon');z.className='upload-zone';
  ...
  const loadGal=function(){
    ...
    storage.listIcons().then(function(icons){
      ...
      icons.forEach(function(ic){
        const card=makeIconChoice(document.createElement('div'),'Use '+ic.name);card.style.cssText='display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px;border:1px solid var(--surface-border);cursor:pointer;background:rgba(0,0,0,0.1);position:relative;';
...
function buildIconsTab(c){
  ...
  function ri(f){
    ...
    if(_mode==='svg'){
      ...
      function renderBatch(){
        ...
        for(;idx<end;idx++){
          var name=items[idx];
          var d=makeIconChoice(document.createElement('div'),'Use '+name);d.className='icon-grid-item';d.style.cssText='padding:6px;';
      ...
      if (efl) {
        ...
        items.forEach(function(emo){
          var d=makeIconChoice(document.createElement('div'),'Use '+emo);d.className='icon-grid-item';
        ...
        categories.forEach(function(cat) {
          ...
          catEmojis.forEach(function(e) {
            var emo = e[0];
            var d = makeIconChoice(document.createElement('div'),'Use '+e[2].split('|')[0]); d.className = 'icon-grid-item';
  ...
  function setMode(m){_mode=m;btnSvg.classList.toggle('selected',m==='svg');btnEmoji.classList.toggle('selected',m==='emoji');btnSvg.setAttribute('aria-pressed',String(m==='svg'));btnEmoji.setAttribute('aria-pressed',String(m==='emoji'));ri(s.value);}
