...
6. **Smart default search** — search module learns your preferred engine per keyword
   prefix (e.g. `yt: cats`, `gh: repo`) via a rules table.
7. **Fuzzy URL completion** — as you type in a search bar, suggest matching links
   already in your dashboard (with favicon) before hitting Enter.
8. **Global quick-jump** — type a link label anywhere (global hotkey) to open it,
   not just within its card.
9. **Natural-language queries** — ask the palette `weather today` / `notes about X`
...
12. **Autocomplete history** — search module keeps a per-engine recent-query history.
13. **Temporal hints** — clock/weather cards can surface a one-line "It's 9:41 AM in
...
18. **Micro-interactions** — button press scale, card hover lift, icon nudges —
...
21. **Consistent surface ladder** — one luminance token ramp for cards/panels/modals
    so contrast is uniform down the page (AURA-015).
22. **Fluid spacing rhythm** — replace hardcoded gaps with the existing `--space-*`
    tokens so all breakpoints share one hierarchy (AURA-004).
23. **Type ramp monotonicity** — CSS owns a strictly monotonic `--text-*` ramp so
...
33. **Reduced-data loading skeletons** — shimmer skeletons on weather/media/git while
...
38. **HTTP caching audit** — confirm immutable `?v=` assets stay cached (done) and
    tighten remaining revalidations.
39. **Icon lazy-loading** — offscreen card icons render when near viewport.
40. **Config write coalescing** — storage.js already coalesces; verify rapid edits
    don't double-write; add a 300ms debounce guard.
41. **JSON parse caching** — cache parsed config keyed by last-modified to skip
    re-normalization on rapid reloads.
42. **DOM batching** — renderAll batches DOM inserts via a fragment (already likely;
    verify + document).
...
49. **Section collapse/expand** — per-section collapse toggle (already for some),
    persisted per card.
...
52. **Notes attachments** — attach an image/file to a note (uploads dir reuse).
53. **Notes search** — Ctrl+F within a notes card highlights matches.
...
56. **API poller richer views** — render JSON as a mini-table or sparkline, not just a
    scalar dot-path.
57. **Resource monitor sparklines** — small inline history charts for CPU/RAM/disk
...
63. **Keyboard help overlay** — a live, searchable shortcuts cheat-sheet (upgrade the
    current help modal).
64. **Command palette accessibility** — full arrow-key + type-ahead + ARIA combobox
    semantics.
...
66. 🎯 **Weather forecast** — multi-day forecast rows (already ships a 5-day outlook;
    add 3-day high/low + condition detail).
...
70. **Homelab service status card** — aggregate ping/HTTP health of your self-hosted
...
76. **Weather radar iframe presets** — one-click embed presets for rain-view maps.
77. **Uptime status card** — pair with the homelab monitor for last-outage history.
78. **Photo of the day card** — optional Bing/NASA APOD background-in-card widget.
79. **Note-to-card templates** — quick templates (meeting notes, grocery list, TODO)
    pre-filled in a notes card.
80. **Markdown card** — a static rendered markdown block (uses item 8's renderer).

## F. Intelligence: Data & Insight (make it *intelligent*)
81. **Usage analytics** — optional local click logging (no external telemetry) with a
    "Most used" and "Recently added" summary in the config panel.
82. **Theme auto-suggest** — sample a background image and suggest an accent color.
83. **Background image optimizer** — auto-compress/downscale uploads (PIL path exists)
...
89. **Import from other dashboards** — accept a simple bookmark-HTML import to seed a
    links section.
90. **Predictive prefetch** — pre-resolve DNS/preconnect to the domains of visible
...
92. **Fetch consolidation onto http.js** — route remaining raw `fetch` calls
    (weather/media/git/storage) through `WarTabHttp` (audit finding).
93. **`stats.js` poller** — use `WarTabHttp.createPoller` instead of raw setInterval.
94. **Dead-code sweep** — remove confirmed-dead helpers (bumpVersion, addGap,
    removeGap, findSection, on/emit bus with no subscribers) after grep-verifying.
95. **Config-default drift fix** — reconcile API_PRESETS URL vs sectionDefaults repo
    (warmbo vs nousresearch) and section-editor style defaults.
96. **Module contract docs** — refresh docs/modules.md with the full 18-module
    contract and the el()/reuse conventions.
97. **Server route hardening audit** — re-verify the SPA allowlist after adding any
    new static file (manifest, sw.js, icons).
98. **CSP headers** — add a baseline Content-Security-Policy that allows the current
    inline styles without breaking custom-CSS or extension.
99. **Test coverage** — add vitest cases for the palette, PWA manifest, context menu,
    and custom-CSS injection; keep `npm run test` green.
100. **README + docs sync** — reconcile README/PLAN with the live schema (pages
    object, glow accent, new modules) so docs match code.
