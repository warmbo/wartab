...
function applyTheme(){
  ...
  if(brand){const b=config.branding||DEFAULT_CONFIG.branding;brand.innerHTML=`<span class="brand-icon">${b.icon||'⚔️'}</span><span>${escAttr(b.title||'WarTab')}</span>`;}
...
function renderStatusBar(){const bar=$('#status-bar'),sb=config.statusBar;if(!sb||!sb.enabled){bar.classList.add('hidden');bar.innerHTML='';return;}bar.classList.remove('hidden');bar.innerHTML='<span class="stat-item"><span class="stat-icon">⚡</span><span class="stat-value" id="stat-loading">Connecting...</span></span>';}
...
function doSearch(q,sec){const s=(q||'').trim();if(!s)return;window.open((config.search.engines[sec.engine||'Google']||config.search.engines['Google'])+encodeURIComponent(s),'_blank');}

function renderSection(sec,card){const f=document.createDocumentFragment();if(sec.label&&sec.type!=='clock'){const l=document.createElement('div');l.className='section-title';l.textContent=sec.label;f.appendChild(l);}switch(sec.type){case'links':{const g=document.createElement('div');g.className='link-grid';(sec.links||[]).forEach(l=>{const a=document.createElement('a');a.className='link-item';a.href=l.url;a.target='_blank';a.rel='noopener';a.appendChild(renderLinkIcon(l.icon));const s=document.createElement('span');s.className='link-label';s.textContent=l.label;a.appendChild(s);g.appendChild(a);});f.appendChild(g);break;}case'link-list':{const l=document.createElement('div');l.className='link-list';(sec.links||[]).forEach(link=>{const a=document.createElement('a');a.className='link-row';a.href=link.url;a.target='_blank';a.rel='noopener';a.appendChild(renderLinkIcon(link.icon));a.appendChild(document.createTextNode(' '+link.label));l.appendChild(a);});f.appendChild(l);break;}case'search':{const w=document.createElement('div');w.style.cssText='display:flex;gap:4px;align-items:stretch;';const wr=document.createElement('div');wr.className='inline-search-wrap';wr.innerHTML='<span class="search-icon">🔍</span>';const i=document.createElement('input');i.type='text';i.placeholder=sec.placeholder||'Search...';wr.appendChild(i);w.appendChild(wr);const b=document.createElement('button');b.className='btn btn-glass btn-search';b.textContent='🔍';b.addEventListener('click',()=>doSearch(i.value,sec));i.addEventListener('keydown',e=>{if(e.key==='Enter')doSearch(i.value,sec);});w.appendChild(b);f.appendChild(w);if(sec.engine){const t=document.createElement('div');t.className='search-engine-tag';t.textContent=sec.engine;f.appendChild(t);}break;}case'clock':{const w=document.createElement('div');w.className='clock-widget';w.dataset.format24=sec.format24h?'1':'0';w.dataset.showDate=sec.showDate?'1':'0';w.innerHTML='<div class="clock-time">--:--</div><div class="clock-date"></div>';f.appendChild(w);break;}case'weather':{const w=document.createElement('div');w.className='weather-widget';w.dataset.apiKey=sec.apiKey||'';w.dataset.location=sec.location||'';w.dataset.units=sec.units||'imperial';w.innerHTML='<span class="weather-icon">🌤</span><div><div class="weather-temp">--°</div><div class="weather-detail">Loading...</div></div>';f.appendChild(w);break;}case'iframe':{const ifr=document.createElement('iframe');ifr.className='card-iframe';ifr.src=sec.url||'';ifr.style.height=(sec.height||300)+'px';ifr.allow='fullscreen';ifr.loading='lazy';f.appendChild(ifr);break;}case'notes':{const e=document.createElement('div');e.className='notes-text';e.contentEditable=true;e.textContent=sec.content||'';e.addEventListener('blur',()=>{const idx=findSection(sec.id);if(idx>=0){config.cards[idx[0]].sections[idx[1]].content=e.textContent;saveConfig();}});f.appendChild(e);break;}case'dropdown':{const b=document.createElement('button');b.className='dropdown-toggle';b.innerHTML=`${escAttr(sec.label||'More')} <span class="arrow">▶</span>`;const c=document.createElement('div');c.className='dropdown-content';b.addEventListener('click',()=>{b.classList.toggle('open');c.classList.toggle('open');});f.appendChild(b);const ig=document.createElement('div');ig.className='link-grid';(sec.links||[]).forEach(link=>{const a=document.createElement('a');a.className='link-item';a.href=link.url;a.target='_blank';a.rel='noopener';a.appendChild(renderLinkIcon(link.icon));const s=document.createElement('span');s.className='link-label';s.textContent=link.label;a.appendChild(s);ig.appendChild(a);});c.appendChild(ig);f.appendChild(c);break;}case'api-poller':{const w=document.createElement('div');w.className='api-widget';w.dataset.url=sec.url||'';w.dataset.jsonPath=sec.jsonPath||'';w.dataset.label=sec.label||'';w.dataset.refresh=sec.refreshInterval||60;w.innerHTML=`<div class="api-row"><span class="api-label">${escAttr(sec.label||'Loading...')}</span><span class="api-value">--</span></div>`;f.appendChild(w);fetchApiWidget(w);break;}}const sl=card.sections||[],is=sl.indexOf(sec)===sl.length-1;if(!is&&sec.type!=='clock'){const hr=document.createElement('hr');hr.className='section-divider';f.appendChild(hr);}return f;}
...
/* ═══════════════════════════════════════════ DRAG ═══════════════════════════════════════════ */
function startDrag(e,id,idx){if(e.button!==0)return;e.preventDefault();const c=config.cards.find(x=>x.id===id);if(!c)return;const g=$('#card-grid'),cel=[...g.children].filter(el=>el.classList.contains('card'));const cl=document.createElement('div');cl.className='card drag-clone';cl.style.cssText=`position:fixed;z-index:999;pointer-events:none;opacity:0.85;width:${cel[0]?.offsetWidth||280}px;transform:rotate(2deg) scale(0.95);`;cl.style.setProperty('--card-accent',c.color||config.theme.glow);cl.innerHTML=`<div style="padding:12px 16px;background:var(--card-bg);backdrop-filter:blur(var(--bg-blur));border:1px solid var(--glass-border);box-shadow:0 12px 48px rgba(0,0,0,.5);"><div style="font-size:13px;font-weight:600;">${c.icon||'📦'} ${escAttr(c.title)}</div></div>`;cl.style.left=(e.clientX-20)+'px';cl.style.top=(e.clientY-10)+'px';document.body.appendChild(cl);const ph=document.createElement('div');ph.className='drag-placeholder';ph.style.height=(cel[idx]?.offsetHeight||120)+'px';g.insertBefore(ph,cel[idx]);cel[idx].style.opacity='0.3';dragState={cardId:id,index:idx,clone:cl,placeholder:ph};document.addEventListener('pointermove',onDragMove);document.addEventListener('pointerup',onDragEnd);document.addEventListener('pointercancel',onDragEnd);}
function onDragMove(e){if(!dragState)return;dragState.clone.style.left=(e.clientX-20)+'px';dragState.clone.style.top=(e.clientY-10)+'px';const g=$('#card-grid'),ph=g.querySelector('.drag-placeholder');if(!ph)return;document.body.style.pointerEvents='none';const eu=document.elementFromPoint(e.clientX,e.clientY);document.body.style.pointerEvents='';const cu=eu?.closest('.card');if(cu&&cu.dataset.cardId!==dragState.cardId){g.insertBefore(ph,cu);}else if(!cu){const cs=[...g.children].filter(el=>el.classList.contains('card')&&el.dataset.cardId!==dragState.cardId);let ins=false;for(const el of cs){const r=el.getBoundingClientRect();if(e.clientY<r.top+r.height/2){g.insertBefore(ph,el);ins=true;break;}}if(!ins)g.appendChild(ph);}}
function onDragEnd(){document.removeEventListener('pointermove',onDragMove);document.removeEventListener('pointerup',onDragEnd);document.removeEventListener('pointercancel',onDragEnd);if(!dragState)return;const{cardId,index,clone,placeholder}=dragState;if(clone.parentNode)clone.remove();const g=$('#card-grid');let ti=-1;if(placeholder&&placeholder.parentNode===g){ti=[...g.children].indexOf(placeholder);placeholder.remove();}const o=g.querySelector(`[data-card-id="${cardId}"]`);if(o)o.style.opacity='';if(ti>=0){const cs=config.cards;const si=cs.findIndex(c=>c.id===cardId);if(si>=0&&si!==ti){const[m]=cs.splice(si,1);const ad=si<ti?ti-1:ti;cs.splice(ad,0,m);saveConfig();renderAll();toast('Card reordered');}else renderAll();}else renderAll();dragState=null;}
