...
registerModule('ascii-anim', {
  ...
  render: (sec,card,cw)=>{
    ...
    WarTabLifecycle.addCleanup(cw,function(){
      ...
      if(_timer){cancelAnimationFrame(_timer);_timer=null;}
      if(ro)ro.disconnect();
      if(sec._asciiRestart===restart)delete sec._asciiRestart;
    });
    function restart(){
  ...
  editor: (sec,card,bd)=>{
    ...
    ],sec.anim||'donut',function(v){sec.anim=v;saveConfig();if(sec._asciiRestart)sec._asciiRestart();}));
    bd.appendChild(cpRange('Speed',parseFloat(sec.speed)||10,1,20,function(v){
      sec.speed=parseInt(v);saveConfig();if(sec._asciiRestart)sec._asciiRestart();
    },1));
    bd.appendChild(cpRange('Contrast',parseFloat(sec.contrast)||10,1,20,function(v){
      sec.contrast=parseInt(v);saveConfig();if(sec._asciiRestart)sec._asciiRestart();
    },1));
    bd.appendChild(cpCheck('Ghost / Afterimage',!!sec.ghost,function(v){
      sec.ghost=v;saveConfig();if(sec._asciiRestart)sec._asciiRestart();
    }));
  },
});
