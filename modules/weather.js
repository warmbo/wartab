...
registerModule('weather', {
  ...
  onMount: function(sec, card, cw) {
    ...
    /* ── Local caching: render last forecast instantly, refresh in background.
         Keyed per-section so multiple weather cards don't collide. The refresh
         interval below keeps the cache warm every 10 minutes. ── */
    var CACHE_TTL = 10 * 60 * 1000;
    var cacheKey = sec.id ? 'wartab.weather.' + sec.id : null;
    ...
    /* ── Render a full weather card from API data (live fetch or cache) ── */
    function renderData(d) {
      var isF = sec.units === 'fahrenheit';
      var cwData = d.current_weather;
      if (!cwData) return false;
    ...
    function fetchWeather() {
      ...
      /* ── Loading state: show skeleton only if nothing rendered yet ── */
    ...
    function doFetch(lat, lon) {
      ...
      /* ── Loading indicator on refresh (only when nothing is rendered) ── */
      if (frame.dataset.weatherReady !== '1') {
        frame.innerHTML = '';
        frame.appendChild(ds.loading(3, 'bar'));
      }
